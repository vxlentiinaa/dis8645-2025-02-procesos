...
void loop() {
  ...
  if (SensorColor::colorDetectado == "ROJO"){
    readColor = 1;
  }
  //intentamos ligar sensor de color y reproductor de audio cambiando el valor de read color frente a cada escenario 
  //de SensorColor usando directamente el resultado de la función del sensor

    if (SensorColor::colorDetectado == "VERDE"){
    readColor = 2;
  }

    if (SensorColor::colorDetectado == "AZUL"){
    readColor = 3;
  }

    if (SensorColor::colorDetectado == "NARANJO"){
    readColor = 4;
  }
