...
Luego me di cuenta de que la pantalla estaba en negro, y era porque los valores del potenciómetro, escritos anteriormente no estaban correctos. Entonces este fue el arreglo:
...
Además se agregaron nuevas imágenes siguiendo la misma lógica, pero la pantalla no lo mostraba pero el error no presentaba ningún error de sintáxis
...
```cpp
 // Mostrar imagen según rango del potenciómetro
  ...
  delay(150); //tiempo de duracion en pantalla
};
```
...
Aquí se configuran los parámetros de la pantalla, las bibliografías y controladores
...
Con el potenciómetro, se elige el pin donde estará para poder hacer la conexión, en este caso es A0
...
Se establece el tamaño del bitmap adecuado a la pantalla que vamos a usar com output, en este caso es una pantalla de 128 x 64 px
...
Aquí es donde se agregar los bitmap creados en esta página:
...
const unsigned char corazonRoto [] PROGMEM = {
...
Se hace un orden de las imagenes del bitmap, para que queden ordenadas y centradas
...
```
 ...
 En el setup, se inicia la conexión con el puerto serial, los bits del potenciómetro, y la muestra de las imagenes
...
En esta parte se le asignan los valores del potenciómetro a cada imagen del bitmap, para que al girarlo, se forme este carrusel de imagenes
...
void loop() {
  ...
  // Mostrar frase según rango del potenciómetro
  if (valorPot < 146) {             // 0 - 145
    showBitmap(ojos);
  } 
  else if (valorPot < 292) {        // 146 - 291
