...
// La segunda patita es GND, cable a tierra (GND) Verde
...
//La primera es GND, conexión a tierra (GND) Verde

//El segundo es VCC (+), alimentación de voltaje de entre 3.3V y 5V (5V) Amarillo

//La tercera es SW, salida del interruptor del botón (pin 7) Naranjo

//La cuarta es DT (output B), se usa para determinar rotación (pin -3) Blanco

//La quinta es CLK (output A), se usa para determinar cantidad de rotación 
//cada ve que el mando es girado en cualquier dirección (pin 2) Rojo
...
// La primera es GND, conexión a tierra (GND) Verde
//La segunda es VCC, pin de alimentación, va entre 1,8V y 6V (5V) amarillo
//La tercera es SCL, pin de señal de reloj de interfaz I2C (A5) Morado
//La cuarta es SDA, pin de señal de datos de interfaz I2C (A4) Azul
